...
const Payment = (
      ...
      try {
        ...
        const oldCustomerMessage: string =
          checkoutInfo !== undefined
            ? checkoutInfo.customerMessage
                .replaceAll('Se requiere factura', '')
                .replaceAll('No se requiere factura', '')
            : '';

        promises.push(
          updateCheckout({ customerMessage: `${orderComment}, ${oldCustomerMessage}` }),
        );
  ...
  const setRequireBill: () => void = () => {
    setState((prevState) => ({ ...prevState, requireBill: !state.requireBill }));
  };
